...
class Solvate:
    def __init__(self, solvent, solute, con, density, solvent2, den2, x,y,z,di, ratio,key):
        ...
        subprocess.run(f"mkdir {self.dir}/Packmol{key}", shell = True)


        with open(f'{self.dir}/Packmol{key}/mixture.inp', 'a') as m:
            info_all = ["#", f"# A mixture of {solvent},{solvent2} and solutes", "#", "","tolerance 2.0", "filetype pdb", f"output {self.dir}/Packmol{key}/Mixture.pdb",
                    "", f"structure {self.dir}/{self.solvent}_Solvent/{self.solvent}_Solvent.pdb", f"  number {self.numberSolvent}", f"  inside box 0. 0. 0. {self.x} {self.y} {self.z}", "end structure","",
                    f"structure {self.dir}/{self.solvent2}_Solvent2/{self.solvent2}_Solvent2.pdb", f"  number {self.numberSolvent2}",
            ...
            for i in range( len(con)):
                ...
                info_all.append(" ")
                info_all.append(f"structure {self.dir}/{solute[i].strip()[:3]}_Solute{i+1}/{solute[i].strip()[:3]}_Solute{i+1}.pdb")
            ...
            info_none=["#", f"# A mixture of {solvent},and solutes", "#", "","tolerance 2.0", "filetype pdb", f"output {self.dir}/Packmol{key}/Mixture.pdb",
                    "", f"structure {self.dir}/{self.solvent}_Solvent/{self.solvent}_Solvent.pdb", f"  number {self.numberSolvent}", f"  inside box 0. 0. 0. {self.x} {self.y} {self.z}", "end structure"]
            ...
            for i in range( len(con)):
                self.molnum = int(
                    float(con[i].strip()) * (6.4e-20)* 6.02214e23*(((self.x/10) * (self.y/10) * (self.z/10))/(40*40*40)))
                info_none.append(" ")
                info_none.append(f"structure {self.dir}/{solute[i].strip()[:3]}_Solute{i+1}/{solute[i].strip()[:3]}_Solute{i+1}.pdb")
        ...
        while not os.path.isfile(f"{self.dir}/Packmol{key}/mixture.inp"):
            time.sleep(1)
        packmol_cmd =f"packmol < {self.dir}/Packmol{key}/mixture.inp"
