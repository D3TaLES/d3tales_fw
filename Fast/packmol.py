    ...
    def __init__(self, solvent, solute, con, density, solvent2, den2, x,y,z,di, ratio,key):
        ...
        with open(f'{self.dir}/Packmol{key}/mixture.inp', 'a') as m:
            ...
            for i in range( len(con)):
                ...
                info_all.append(" ")
                info_all.append(f"structure {self.dir}/{solute[i].strip()}_Solute{i+1}/{solute[i].strip()}_Solute{i+1}.pdb")
            ...
            for i in range( len(con)):
                self.molnum = int(
                    float(con[i].strip()) * (6.4e-20)* 6.02214e23*(((self.x/10) * (self.y/10) * (self.z/10))/(40*40*40)))
                info_none.append(" ")
                info_none.append(f"structure {self.dir}/{solute[i].strip()}_Solute{i+1}/{solute[i].strip()}_Solute{i+1}.pdb")
