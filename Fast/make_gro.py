...
class gro:
    def __init__(self, solvent, solute, solvent2, di, x, y, z,key):
        ...
        command4 = f'mv {self.dir}/{solvent}_Solvent/{solvent}_Solvent.pdb {self.dir}/InputGrofiles{key} && mv {self.dir}/{solvent}_Solvent/{solvent}_Solvent.gmx.itp {self.dir}/InputGrofiles{key} && mv {self.dir}/{solvent}_Solventscript {self.dir}/InputGrofiles{key}'
        ...
        for i in range(len(self.solute)):
            com1 = (
    f"mv {self.dir}/{self.solute[i].strip()[:3]}_Solute{i+1}/"
    f"{self.solute[i].strip()[:3]}_Solute{i+1}.pdb {self.dir}/InputGrofiles{key} && "
    f"mv {self.dir}/{self.solute[i].strip()[:3]}_Solute{i+1}/"
    f"{self.solute[i].strip()[:3]}_Solute{i+1}.gmx.itp {self.dir}/InputGrofiles{key} && "
    f"mv {self.dir}/{self.solute[i].strip()[:3]}_Solute{i+1}script {self.dir}/InputGrofiles{key}")
