...
class toopol:
    def __init__(self, solvent1, solvent1N, solvent2, solvent2N, solute, con, currentdir, x,y,z,key):
        ...
        if os.path.isdir(f'{self.dir}/InputGrofiles{key}') == True:
            ...
            while True:
                if os.path.isdir(f'{self.dir}/Old_InputGrofiles_Output/InputGrofiles{folder_number+1}{key}'):
                    folder_number +=1
                else:
                    move = f'mv {self.dir}/InputGrofiles {self.dir}/Old_InputGrofiles_Output/InputGrofiles{folder_number+1}{key}'
                    subprocess.run(move, shell=True, check=True)
                    break
        if os.path.isdir(f'{self.dir}/Output{key}') == True:
            folder_number = 0
            while True:
                if os.path.isdir(f'{self.dir}/Old_InputGrofiles_Output/Output{folder_number + 1}{key}'):
                    folder_number += 1
                else:
                    move = f'mv {self.dir}/Output {self.dir}/Old_InputGrofiles_Output/Output{folder_number + 1}{key}'
                    subprocess.run(move, shell=True, check=True)
                    break
        subprocess.run(command2, shell=True, check=True)
        cmd = f"touch {self.dir}/InputGrofiles{key}/topol.top && touch {self.dir}/InputGrofiles{key}/nmol.itp"
        subprocess.run(cmd, shell=True, check=True)
        with open(f"{self.dir}/InputGrofiles{key}/topol.top", 'a') as top, open(
                f"{self.dir}/InputGrofiles{key}/nmol.itp", 'a') as nmol:
