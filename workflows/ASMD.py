...
class ASMD:
    def __init__(self):
        ...
        self.input_dir = os.path.join(self.current_dir, 'InputGrofiles')
        self.output_dir = os.path.join(self.current_dir, 'Output')
        self.mdp_dir = os.path.join(self.current_dir, 'Fast/MDP')
        ...
        self.xtc_file = os.path.join(self.output_dir, "production.xtc")
        self.tpr_file = os.path.join(self.output_dir, "production.tpr")
        self.eqtpr_file = os.path.join(self.output_dir, "equilibration.tpr")
        self.trr_file = os.path.join(self.output_dir, "production.trr")
        self.nmol_itp = os.path.join(self.input_dir, "nmol.itp")
        self.gro_file = os.path.join(self.output_dir, "production.gro")
        self.edr_file = os.path.join(self.output_dir, "equilibration.edr")
    ...
    # 2
    def EnergyMin(self):
        command = ["gmx_mpi", "grompp", "-f", os.path.join(self.mdp_dir, "em.mdp"), "-c", self.initial_coordinates,
    ...
    # 3
    def NVT(self):
        command = ["gmx_mpi", "grompp", "-f", os.path.join(self.mdp_dir, "nvt.mdp"), "-c",
                   os.path.join(self.output_dir, "em.gro"), "-p",
    ...
    def NPT(self):
        command = ["gmx_mpi", "grompp", "-f", os.path.join(self.mdp_dir, "equilibration.mdp"), "-c",
                   os.path.join(self.output_dir, "nvt.gro"), "-t", os.path.join(self.output_dir, "nvt.cpt"), "-p",
                   self.topology_file, "-o",
    ...
    def production_run(self, topology_file, output_dir):
        ...
        command = ["gmx_mpi", "grompp", "-f", os.path.join(self.mdp_dir, "production.mdp"), "-c",
                   os.path.join(output_dir, "equilibration.gro"), "-t", os.path.join(output_dir, "equilibration.cpt"),
                   "-p",
                   topology_file, "-o",
    ...
    def msd_mkaer(self):
        subprocess.run(
            ["gmx_mpi", "msd", "-f", self.xtc_file, "-s", self.tpr_file, "-n", f"{self.output_dir}/index.ndx", "-o",
